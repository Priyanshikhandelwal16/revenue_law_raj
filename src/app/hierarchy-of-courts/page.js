      ...
      <div style={{
        ...
        <div className="layout-container">
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255, 255, 255, 0.25)', borderRadius: '50px', padding: '0.35rem 1rem', marginBottom: '1.5rem' }}>
            ...
            <span style={{ fontSize: '0.8rem', color: '#FFFFFF', fontWeight: 600, letterSpacing: '1px', textTransform: 'uppercase' }}>Court Directory</span>
          </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem', marginBottom: '4rem' }}>
              {courts.map((court, idx) => (
                <div key={idx} style={{ width: '100%', maxWidth: '750px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  ...
                  {idx < courts.length - 1 && (
                    <div style={{ margin: '1.25rem 0', display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative' }}>
                      <div style={{ width: '3px', height: '35px', background: 'linear-gradient(to bottom, var(--accent-gold), rgba(197, 168, 128, 0.4))', borderRadius: '4px' }}></div>
                      <ChevronDown size={22} style={{ color: 'var(--accent-gold)', marginTop: '-8px', filter: 'drop-shadow(0px 2px 4px rgba(0,0,0,0.1))' }} />
                    </div>
                  )}
                </div>
              ))}
